...
router.get('/updateRoleAcceptRefuse/:id', checkValidTokenAdmin, function (req, res, next) {
  console.log("accept_updateRole_id")
  console.log(req.query)
  if (req.query.accept != undefined && req.query.accept == "true") {
    RequestUpdateRole.lookup(req.params.id)
      ...
      .catch(error => {
        res.status(401).jsonp({ error: "Não encontrou o pedido!" });
      });
  }else{
    ...
    .catch(error => {
      res.status(402).jsonp({ error: "Não encontrou o pedido!" });
    })
    } else
      res.status(403).jsonp({ error: "Não encontrou o pedido!" });
  }
});
